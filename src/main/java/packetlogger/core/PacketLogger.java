	...
	public static void logPacket(Packet packet)
	{
		...
		if ( packet instanceof S35PacketUpdateTileEntity )
		{
			...
			if ( tag == null )
				...
				log.info( "S35PacketUpdateTileEntity: @" + p.func_148856_c() + "," + p.func_148855_d() + "," + p.func_148854_e() + " containing "
						+ tag.toString() );
		}
		else if ( packet instanceof S3FPacketCustomPayload )
		{
			S3FPacketCustomPayload p = (S3FPacketCustomPayload) packet;
			log.info( "S3FPacketCustomPayload: " + p.func_149169_c() + " containing " + p.func_149168_d().length + " bytes" );
		}
